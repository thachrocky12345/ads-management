...
_COMPLEXITY_MULTIPLIERS: dict[TaskComplexity, float] = {
    TaskComplexity.LOW:    0.65,
    TaskComplexity.NORMAL: 1.00,
    TaskComplexity.HIGH:   1.45,
}
...
def get_agent_configs() -> list[dict]:
    """Return agent configurations for the API."""
    configs = []
    for agent_id, base in _BASE_BUDGETS.items():
...
class TokenBudgetManager:
    ...
    def record_actual_usage(
        self, agent_id: str, tokens_used: int, budget: TokenBudget
    ) -> None:
        """Track real usage for future budget auto-tuning."""
        utilization = tokens_used / budget.total_budget if budget.total_budget > 0 else 0
