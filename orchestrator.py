...
COVERAGE_THRESHOLDS = {
    "min_to_advance":   0.80,   # 80%+ → use result, add warning
    "min_to_retry":     0.50,   # 50–79% → retry with more budget
    # Below 50% → replan from scratch
}

# How many times to retry each agent before escalating
...
class Orchestrator:
    ...
    async def run_pipeline(
        ...
        Returns a complete PipelineState with all results.
        """
    ...
    def decide(self, result: AgentResult) -> OrchestratorDecision:
        ...
        if result.status == CompletionStatus.PARTIAL_SAFE:
            ...
            if coverage >= COVERAGE_THRESHOLDS["min_to_advance"]:
                # 80%+ done — good enough, note it in report
                warning = (
                ...
                # 50–79% — retry from checkpoint if available
                if result.checkpoint and retry_count < MAX_RETRIES_PER_AGENT:
                    ...
                    # Exhausted retries — use what we have with warning
                    warning = (
                ...
                # Under 50% — too little to be useful, replan
                logger.error(
        ...
        if result.status == CompletionStatus.PARTIAL_UNSAFE:
            # Agent stopped mid-calculation — data is corrupt, never use it
            logger.error(
    ...
    async def _run_agent_with_budget(
        ...
        The budget is always assigned BEFORE the agent starts.
        """
        budget = override_budget or self.budget_manager.assign(agent_id, complexity)
        task["_budget"] = budget   # Budget travels with the task
